...
/**
 * @brief Reads the temperature from an Adafruit MAX31856 (or compatible)
 *thermocouple breakout board. Communicates with the ESP via SPI.
 *
 * @param cs_pin
 * @param mosi_pin
 * @param miso_pin
 * @param clk_pin
 * @param drdy_pin The data-ready pin. No longer used, but still provided for
 ...
 **/
class MAX31856 : public SensorT<float> {
 public:
  /* DEPRECATED: drdy_pin no longer used */
  MAX31856(int8_t cs_pin, int8_t mosi_pin, int8_t miso_pin, int8_t clk_pin,
           int8_t drdy_pin, max31856_thermocoupletype_t tc_type,
           uint read_delay, String config_path = "");

  /**
   * A version of the constructor that doesn't require the now-obsolete
   *drdy_pin.
   **/
  MAX31856(int8_t cs_pin, int8_t mosi_pin, int8_t miso_pin, int8_t clk_pin,
           max31856_thermocoupletype_t tc_type, uint read_delay,
           String config_path = "")
      : MAX31856(cs_pin, mosi_pin, miso_pin, clk_pin, 0, tc_type, read_delay,
                 config_path) {}

  /**
   * A version of the constructor that allows you to create the
  Adafruit_MAX31856
   ...
   * like this:
     ...
     MAX31856(max, 1000, "/config/path");

   * @param read_delay How often to read the sensor, in ms.
   * @param config_path The path to configure read_delay in the Config UI.
  **/
  MAX31856(Adafruit_MAX31856* max31856, uint read_delay = 500,
           String config_path = "");
